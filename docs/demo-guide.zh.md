...
| 客户门户 | /portal | 我的订单、下单、估价、确认运输方案、预报入库 /portal/asns（只读）、上传入库清单 /portal/asns/imports/create（CSV / Excel → 预览 → 确认生成订单，模板 /portal/asns/imports/template，已提交清单 /portal/asns/imports）、我的库存 /portal/stock(含声明的存储等级,不显示库位)、我的账单 /portal/invoices、我的报表 /portal/reports |
...
| 客户想让我们去取货运到仓库怎么办 | 不要建"现场提货直送"订单（那是给客户送货的订单，没有收件人、货不进库、按出库操作费计）。正确做法：预报单 (ASN) → 到仓方式选"我方上门提货"，填提货地址 / 联系人、可提货日期和申报包裹（类型、件数、单件重量、尺寸；单件 ≥ 尾板阈值自动加尾板）→ 提交后运输部在运单页看到"入库提货 → 仓库"的运单并报价（自有车队 / 承运商）→ 调度确认方案（v1 不在门户由客户选）→ 订舱 → 司机在 /driver 看到提货地址去取 → 到仓签收 POD 即预报单"已到货" → 照常收货上架。费用 = 运费（含尾板 / 燃油 / 偏远，记到预报单的 Job）+ 入库费；不产生出库操作费。订舱前可在预报单页修改或"改为客户自送"；订舱后由调度在运单页处理。客户门户的预报入库页只读显示提货进度和运费客户价。整柜 / 拼柜的拖车仍走"物理柜" |
