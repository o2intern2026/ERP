...
4. **运输**(dispatcher):/transport 打开该运单 → 方案列表(自有车队 / Karrio / 人工录价)→ 选一个 → "订舱"。自有车队:/transport/runs/create 建今天的班次、加入运单;然后 warehouse-operator 在 /warehouse/outbound "发运交接"。方案列表上方有"重新报价"（方案 24 小时有效，过期的行标"已过期"并显示"有效至"；周五打包周一订舱就点它，客户已选方案在容差内会自动再确认）。班次页可"修改班次"（日期 / 司机 / 车辆）、把某一站"移出班次"（运单保持已预订，可加到别的班次）或"取消班次"——只要还没有站点签收（CR #133）。
5. **司机签收**(transport-operator,手机或电脑):/driver → 选停靠点 → 签名 + 拍照 → 送达。订单变"已送达";签收邮件内容在 `storage/logs/laravel.log`。昨天没跑完的班次今天还在页面上（标"配送日期 … 尚未完成"），送完最后一站自动完成；明天的班次要到明天才出现（CR #133）。
