...
8. **客户视角**(client):/portal 只看到 Edward 的订单;下单、估价、确认运输方案、看库存、下载发票;"预报入库"里自己上传装箱单 + 柜号 + 预计到港日生成预报单（待客服确认），并下载入库单 PDF。
...
| 客户怎么自己报货（不用客服建预报单） | 客户账号登录门户 → 顶栏"预报入库" → "新建预报"：选仓库、入库类型，填柜号 / 柜型、预计到港日、参考号，上传装箱单（xlsx / csv，页面有 CSV 模板；唛头、品名、箱数必填）→ 提交即生成预报单 (ASN)，状态"待客服确认"。客服 / 管理员在 仓库 → 预报单 (ASN) 看到角标和"确认预报单"按钮（列表可勾"只看待确认的客户预报"）；确认前客户可在详情页整份替换装箱单。有系统的客户用订单 API 同一个 token 向 `POST /portal/api/asns` 推送 JSON（`inbound_type`、`expected_date`、`containers[]`、`lines[]`，可带 `Idempotency-Key`）。收货、上架不等确认，规则没变 |
