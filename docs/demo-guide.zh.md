...
| 客户想让我们去取货运到仓库怎么办 | 不要建"现场提货直送"订单（那是给客户送货的订单，没有收件人、货不进库、按出库操作费计）。**客户自己申请（推荐）**：客户在门户"上传入库清单"时勾选"需要你们上门提货"，填提货地址、联系人、可提货日期（包裹按清单每行的箱数 / 重量 / 尺寸自动计算，不用再填）→ 预览页列出提货方案和客户价，客户勾选一个后"确认提交" → 客服在"待建预报"看到"客户申请上门提货"标记、提货城区 / 日期和客户所选方案，点"选中并填入"会勾选订单并把提货信息、仓库带入（可修改）→ 生成预报单时自动提交运输报价（客服不生成预报单，运输不会有任何动作）→ 最终报价与客户所选方案的差异在容差内（默认 10%）系统按客户的选择自动确认；超出时客户在门户"预报入库"页的"提货运费有变化，请确认方案"里重新确认（或调度 / 客服在运单页代确认）。**客服直接发起**：预报单 (ASN) → 到仓方式选"我方上门提货"，填提货地址 / 联系人、可提货日期和申报包裹（类型、件数、单件重量、尺寸；单件 ≥ 尾板阈值自动加尾板）→ 调度在运单页确认方案。之后都一样：订舱 → 司机在 /driver 看到提货地址去取 → 到仓签收 POD 即预报单"已到货" → 照常收货上架。费用 = 运费（含尾板 / 燃油 / 偏远，记到预报单的 Job）+ 入库费；不产生出库操作费。订舱前可在预报单页修改或"改为客户自送"（修改会重新报价，客户选过方案的仍按其选择在容差内自动确认）；订舱后由调度在运单页处理。在预报单页"从订单导入货物行"时，来自客户申请了上门提货的清单的订单会显示提醒，导入后请在到仓方式中设置。整柜 / 拼柜的拖车仍走"物理柜" |
