...
@section('title', __('masterdata.carriers.title'))

@section('content')
    <h1>{{ __('masterdata.title') }}</h1>
    @include('masterdata::partials.tabs', ['active' => 'carriers'])
    <p style="text-align:right"><a role="button" href="{{ route('masterdata.carriers.create') }}">{{ __('masterdata.carriers.create') }}</a></p>
            ...
            <tbody>
                @forelse ($carriers as $row)
                    <tr>
                        ...
                        <td><a href="{{ route('masterdata.carriers.edit', $row) }}">{{ __('platform.common.edit') }}</a></td>
                    </tr>
                @empty
