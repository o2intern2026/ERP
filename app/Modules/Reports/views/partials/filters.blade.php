...
<form method="get" action="{{ $action }}" class="grid">
    ...
    <label>{{ __('reports.filters.from') }}<input type="date" lang="en-AU" name="from" value="{{ old('from', $period->from->toDateString()) }}" required></label>
    <label>{{ __('reports.filters.to') }}<input type="date" lang="en-AU" name="to" value="{{ old('to', $period->to->toDateString()) }}" required></label>
