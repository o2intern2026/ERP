...
<form method="get" action="{{ $action }}" class="grid">
    ...
    <label>{{ __('reports.filters.from') }}<x-date-field name="from" value="{{ old('from', $period->from->toDateString()) }}" required /></label>
    <label>{{ __('reports.filters.to') }}<x-date-field name="to" value="{{ old('to', $period->to->toDateString()) }}" required /></label>
