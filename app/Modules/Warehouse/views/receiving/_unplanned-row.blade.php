...
<tr class="receipt-row" data-index="{{ $index }}">
    ...
    <td class="type"><select name="{{ $name('pallet_source') }}" class="pallet-source" aria-label="{{ __('warehouse.receiving.pallet_source') }}">@foreach ($palletSources as $ps)<option value="{{ $ps }}" @selected(($row['pallet_source'] ?? 'client_own') === $ps)>{{ __('warehouse.pallet_sources.'.$ps) }}</option>@endforeach</select></td>
