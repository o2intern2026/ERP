...
class QuoteController extends Controller
{
    ...
    public function store(Request $request, QuoteService $quotes): RedirectResponse
    {
        $data = $request->validate([
            ...
            'lines.*.qty' => ['exclude_without:lines.*.charge_code', 'required', 'numeric', 'gt:0'], 'lines.*.weight_kg' => ['nullable', 'numeric', 'min:0'], 'lines.*.cost' => ['nullable', 'numeric', 'min:0'], 'lines.*.base' => ['nullable', 'numeric', 'min:0'],
        ]);
        $lines = collect($data['lines'])->filter(fn ($l) => ! empty($l['charge_code']))
            ->map(fn ($l) => ['charge_code' => $l['charge_code'], 'qty' => (float) $l['qty'], 'context' => array_filter(['weight_kg' => $l['weight_kg'] ?? null, 'cost_cents' => isset($l['cost']) && $l['cost'] !== '' ? (int) round($l['cost'] * 100) : null, 'base_cents' => isset($l['base']) && $l['base'] !== '' ? (int) round($l['base'] * 100) : null], fn ($v) => $v !== null)])->values()->all();
        if ($lines === []) {
