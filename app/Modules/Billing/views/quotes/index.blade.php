...
@section('content')
    ...
    @else
        <table class="dense">
            ...
            <tbody>@foreach ($quotes as $q)<tr><td><a href="{{ route('billing.quotes.show', $q) }}">{{ $q->quote_no }}</a></td><td>{{ $q->client->name }}</td><td>{{ __('billing.quotes.stages.'.$q->stage) }}</td><td>{{ $q->valid_until?->format('Y-m-d') }}</td><td class="num">{{ number_format($q->total_cents / 100, 2) }}</td><td><span class="badge" data-tone="{{ ['draft' => 'muted', 'sent' => 'warn', 'accepted' => 'ok', 'rejected' => 'danger', 'expired' => 'muted'][$q->status] }}">{{ __('billing.quotes.statuses.'.$q->status) }}</span></td></tr>@endforeach</tbody>
        </table>
        {{ $quotes->links() }}
    @endif
@endsection
