...
@section('content')
    ...
    <table class="dense">
        ...
        @foreach ($quote->lines as $l)
            <tr><td><code>{{ $l->charge_code }}</code></td><td>{{ $l->description }}</td><td class="num">{{ rtrim(rtrim(number_format($l->qty, 3), '0'), '.') }}</td><td>{{ $l->uom }}</td><td class="num">{{ number_format($l->amount_cents / 100, 2) }} @if (($l->assumptions['is_poa'] ?? false) || ($l->assumptions['missing_rate'] ?? false))<br><span class="badge" data-tone="danger">{{ __('billing.quotes.poa_flag') }}</span>@endif</td><td><small><code>{{ json_encode(collect($l->assumptions)->except(['calculation'])->all(), JSON_UNESCAPED_UNICODE) }}</code></small></td></tr>
        @endforeach
        </tbody>
        <tfoot><tr><td colspan="4"><strong>{{ __('billing.quotes.total') }}</strong></td><td class="num"><strong>{{ number_format($quote->total_cents / 100, 2) }}</strong> <small class="text-muted">({{ __('billing.invoices.gst') }} {{ number_format($quote->gst_cents / 100, 2) }})</small></td><td></td></tr></tfoot>
    </table>
