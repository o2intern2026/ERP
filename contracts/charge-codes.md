...
| — ※ | Storage – bottom-level location surcharge | `WH-STORAGE-TIER-PLT-WK` | storage | pallet_week | snapshot.weekly | weeks (1) | CHANGE_REQUESTS #126: a GOOD pallet whose LAST snapshot of the week has location_type=storage, location_storage_tier=bottom AND required_storage_tier=bottom; `pricing_mode = percent` of the same pallet's base storage charge that week (`context.base_cents` = its amount; a missing or POA / needs_review base → no base_cents → the surcharge is POA / needs review, never $0); same key `unit:{stock_unit_id}:week:{week}`, dated the Sunday; `{"min_percent":10,"max_percent":20}`; a warehouse row (MEL / SYD) beats the all-warehouse row; partly picked pallets pay in full; cartons, pickface and quarantine never; GST 10 % | 10 % (standard card seed, no Edward row) |
...
| `min_percent`, `max_percent` | WH-STORAGE-TIER-PLT-WK | the band `markup_percent` must stay within (10 / 20) — the rate-card form reads it from the item's own `threshold_json`, else from the bound standard card's item for the code (CHANGE_REQUESTS #126) |
| `tier_value_threshold_cents` | WH-STORAGE-TIER-PLT-WK | declared unit price in cents (`order_lines.unit_price_cents`, sheet 单价) at or above which an import without an explicit 存储等级 pre-fills `storage_tier = bottom`, source `value_rule`, with a warning; **no default** (absent → no pre-fill); read by `OrderImportService` through `RateService::thresholds`; never prices anything (#126, lead answer 4) |
