...
- **Intake (module service `ShipmentIntakeService`, CHANGE_REQUESTS #124):** `fromAsnCollection(array $envelope): ?Shipment` — `asn.collection_requested` → the ONE `inbound_collection` shipment of the ASN (`asn_id`, `order_id` null, sender = `collection_address`, receiver = the ASN's warehouse, items = declared packages else goods lines, zone = pickup postcode, tailgate at pickup by the client's `TR-TAILGATE` threshold), quoted at the `final` stage at once; a higher `activity_version` re-quotes the same shipment (status back to `quoting`, earlier quotes `requoted`, selection cleared), an older / equal one is a replay, a booked one refuses and logs. `cancelAsnCollection(array $envelope): ?Shipment` — `asn.collection_cancelled` → `booking_cancelled` while not booked. No client choice for collections in v1: `applyClientPreference` is skipped (no order), the dispatcher / customer service confirms on the shipment page.
...
clearCollection(Asn $asn, ?int $userId, ?string $reason = null): Asn        // 改为客户自送: refused once booked (`locked`); inbound_transport = client_delivers, status / shipment / plan cleared, publishes asn.collection_cancelled
