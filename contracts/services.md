...
- `BillingChargeConsumer` is registered for `task.completed`, `asn.putaway_completed`, `outbound.packed`, `shipment.quote_confirmed`, `delivery.extra_charge`. Payload fields read: see `charge-codes.md` quantity sources plus `activity_version` (redo counter), `carrier_cost_cents` / `client_price_cents` / `zone` / `carrier_id` / `service_level` / `tailgate_required` / `charge_code`, and for `order_type = pickup_deliver`: `order_type`, `is_urgent`, `lines[].unit_type|qty|unit_weight_kg`, `pallet_count`, `label_count` (shipment.quote_confirmed — CHANGE_REQUESTS #120), `charge_type` (delivery.extra_charge), `lines[].unit_type|qty|unit_weight_kg`, `is_urgent`, `label_count` (outbound.packed; shipment.quote_confirmed for pickup_deliver).
