...
| `asns` | id, asn_no (unique), job_id, client_id, warehouse_id, expected_date, inbound_type, status, created_by_type, created_by, unplanned (bool), unplanned_confirmed (bool), client_confirmed_at / client_confirmed_by (nullable — set when customer service confirms a client-submitted ASN, i.e. created_by_type = client from the portal form or API push; CHANGE_REQUESTS #116), arrived_at, receiving_completed_at (nullable — set when a 入库单 batch completes and every line has been received; CHANGE_REQUESTS #90), putaway_completed_at, closed_at, notes, timestamps; **到仓方式 (CHANGE_REQUESTS #124):** inbound_transport (`client_delivers` default \| `we_collect`), collection_address (json {name, phone, address, suburb, state, postcode, type}), collection_ready_date, collection_packages (json list {package_type, qty, weight_kg per piece, length_mm, width_mm, height_mm}), collection_notes, collection_requested_at, collection_requested_by (→ users), collection_version (0 = never requested; = the event's activity_version), collection_shipment_id (Transport's shipments.id, no FK — copied back from Transport's events), collection_status (display only, from Transport's events), collection_plan (json: shipment_no, source, carrier_id, carrier_name, service_level, customer_price_cents, eta_days, confirmed_at, booking_ref, tracking_number — the CLIENT price only, never cost); **客户门户申请上门提货 (CHANGE_REQUESTS #125):** collection_preference (json nullable: carrier_id, carrier_name, source, service_level, customer_price_cents, eta_days, is_recommended, is_cheapest, is_fastest, chosen_at, chosen_by — the plan the client ticked in the portal, customer fields only; Transport reads it read-only as the final-quote reference), collection_requested_via (string 10 nullable: client \| staff), collection_import_id (nullable, indexed → Orders `order_imports.id`, no FK). **Terminology: ASN = 预报单 (pre-advice); 入库单 = `goods_receipts` (lead decision 2026-09-08, #92)** |
| `containers` | id, asn_id, job_id, physical_container_id (nullable → physical_containers; NULL = the single-client / FCL path, untouched — CHANGE_REQUESTS #122), container_no, size, unpack_mode, gross_weight_kg, line_count (derived), devanning_basis / devanning_basis_qty / devanning_share (decimal 6,4) / devanning_basis_provisional (bool) — snapshot of the row's last share of its box's devanning / cartage fee (#122), timestamps — basic fields only, no lifecycle |
| `physical_containers` | id, container_no (index; unique with eta_date — a number is reused across voyages), warehouse_id, size, unpack_mode (box-level; a member row keeps its own and is priced by it), gross_weight_kg (the 22.5 t cartage cap), consolidation (`fcl` \| `lcl`, derived = distinct clients of the members, never edited), allocation_basis, cartage_by_us (bool), sideloader_required (bool), eta_date, arrived_at, devanned_at, status, devanning_task_id (nullable → warehouse_tasks: the ONE box-level devanning task), allocation_version (0 = nothing emitted; every task.completed / physical_container.arrived of the box carries it as activity_version, 重算分摊 bumps it), allocation_stale (bool: members changed after an emission), arrived_event_id (uuid, nullable), notes, created_by, timestamps; index (warehouse_id, status). **No client_id** — the box spans clients: staff routes only, never loaded from a portal route (CHANGE_REQUESTS #122, 拼柜 / LCL). Basic fields only — no seals, customs, CFS lifecycle |
...
| `warehouse_tasks` | id, task_no (unique), task_type, job_id, client_id (both **nullable since #122, NULL only for source_type = physical_container** — the box-level devanning task spans clients; every other task carries both), warehouse_id, source_type, source_id, order_id, fulfilment_id, wave_id (nullable, M4), asn_id, container_id, physical_container_id (nullable → physical_containers, #122), priority, assigned_user_id, status, exception_reason, cancel_reason, billable_qty, billable_uom, hours_business, hours_after_hours, notes, started_at, completed_at, completed_by, billable_event_id (→ outbox_events.event_id), timestamps |
...
| `shipments` | id, shipment_no (unique), job_id, client_id, order_id (**nullable since #124** — null for an `inbound_collection`), asn_id (nullable → Warehouse `asns`, no FK; the 预报单 of an `inbound_collection`, #124), asn_activity_version (nullable; = the last `asn.collection_requested.activity_version` applied, #124), fulfilment_id (nullable), shipment_type, status, selected_quote_id (nullable → transport_quotes), carrier_id, service_level, booking_ref, tracking_number, waybill_document_id, consignment_note_document_id, tailgate_required (bool), delivery_run_id (nullable), dispatched_at, delivered_at, timestamps |
...
C: §0, §1, §2, §4, §6 tables. X1: §3 tables. X2: §5 tables. Cross-module writes: **one, and only in Platform** — `JobService::moveOrder` (services.md §5, CHANGE_REQUESTS #119) rewrites `job_id` on `orders` (§3), `customer_quotes` (§6), `shipments` + `carrier_costs` (§5), `packages`, `outbound_dispatches`, `warehouse_tasks` (§4) and the order's `exceptions` / `documents` (§1) when an order is merged into another Job of the same client (从订单生成预报单 / 从订单导入货物行); owners of those tables treat `job_id` as re-homable by that one call and nothing else. Otherwise none — Warehouse never writes `charges`; Billing never writes `stock_ledger`; nobody writes `jobs`, `exceptions`, `documents`, `outbox_events` except through Platform services (§0.2 rules 2–3).
