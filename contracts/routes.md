...
| Portal | `GET /portal/asns`, `GET /portal/asns/create`, `GET /portal/asns/template`, `POST /portal/asns`, `GET /portal/asns/{asn}`, `POST /portal/asns/{asn}/packing-list` (客户自助预报入库, CHANGE_REQUESTS #116: the client's own ASNs; submit = header + container + ETA + packing list → Warehouse AsnService::create (created_by_type client) + AsnImportService::import, logged in `portal_asn_submissions`; detail shows lines, parse results and the 入库单 PDFs via DocumentDownloader; 补传 replaces the draft lines through AsnService::clearDraftLines while unconfirmed; CSV template of the parser's headers) | `portal.asns.index`, `portal.asns.create`, `portal.asns.template`, `portal.asns.store`, `portal.asns.show`, `portal.asns.import` | `portal::asns.index`, `portal::asns.create`, `portal::asns.show` |
| Portal | `POST /portal/api/asns` (token-authenticated JSON push of an ASN — the client API token from `/orders/api-tokens`, optional `Idempotency-Key` replays the first ASN; header + `containers[]` + `lines[]`; without session, CSRF or client.scope like `/orders/api/orders`; CHANGE_REQUESTS #116) | `portal.api.asns.store` | — (JSON) |
