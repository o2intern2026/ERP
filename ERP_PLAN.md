...
**取舍:** **采用**:Job 工作台(Platform 提供 `JobService` 与页面骨架,各模块贡献自己的面板)、Exception Centre(统一 `exceptions` 表,取代仅 OMS 的 协调员队列)、统一 `documents` 表(类型、关联对象、client 可见性)、Integration Monitoring(TD/EIZ 调用与事件失败的重试与告警)、Global Search(Job、订单、唛头、柜号、tracking、发票)。安全、备份、性能、可观测性作为工程基线(§2.6)。**不采用**:导入历史泛化(各导入自记历史即可)、门户角色矩阵(客户角色暂只一种,细分待后续;成本与毛利对客户不可见的规则在 PLT-1 保持)。
...
| 异常中心 | OMS / WMS / TMS / Billing 的异常统一列表:收货差异、Pick Short、配送失败、Missing Rate、Billing Hold、集成失败 | **OMS-2 / TMS-3 / FIN-2** 的异常处理入口 — 取代仅 OMS 的 协调员队列 |
...
- 统一 `exceptions` 表(类型、来源模块、关联 Job 与单据、状态、处理人);收货差异、Pick Short、配送失败、Manual Transport、Missing Rate、Billing Hold、集成失败全部接入;取代仅 OMS 的 协调员队列(A15 变为异常中心的一个筛选视图)。
...
| 协调员队列 | 订单列表的预设视图:待确认 / 缺货 / 财务锁 / 异常 / 今日待发 | **OMS-2 订单状态** — 调度员统一管理待处理订单与异常,不必自己记 |
...
| A15 | ✅ M3 · 协调员队列 | A3 | CartonCloud live queue |
...
- 2026-09-14 CR #123:客户门户也走同一条导入路径——客户在 预报入库 上传 CSV / Excel(可带柜号 / 柜型 / 预计到港日 / 参考号)→ 预览 → 确认生成自己的订单(来源 portal、同一清单一个 Job),客服在 待建预报 看到客户填的入库信息并一键带入生成预报单;解析器补齐编码 / 分隔符 / 全角 / 表头别名 / 单位 / 电话 / 邮编 / 州 的准确转换,逐行中文报错带行号与列名(`contracts/services.md` §8)。
- 2026-09-17 CR #128:门户 预报入库 增加 手工建立入库清单——页面逐行填货物(字段同 CSV 模板,`ManifestParser::fromRows` 走与文件完全相同的规范化 / 校验,行级中文报错回到表单)并可勾选已下、尚无预报单的订单带入同一份清单(**以订单为准**:订单的收件人 / 货物不变,清单只补入库信息和上门提货申请)→ 同一预览 / 确认 → 待建预报 一张卡片(新生成 + 带入);支持草稿。不新增表:`order_imports.status` +draft,行与勾选在 `errors.context.manual`,带入结果在 `errors.result.attached`。
...
| **Q3** | **OMS-3 定价 = 服务费预估** —— 按客户价目表估算仓储/操作/运费,下单时可见 | 任务 A7b 调 `RateService` 做预览;订单行里的 `unit_price / total_price`(货值 AUD)只存不算,仅供报关与保险参考;**CR #126(2026-09-15 lead):单价可按客户价目表门槛 `tier_value_threshold_cents` 预选“底层”存储等级,仍不参与任何计价**;**CR #129(2026-09-17 lead):下单时每行货物可直接选存储等级(标准 / 底层——门户表单、客服手工建单、订单 API、草稿行增改),来源 client / staff,随待建预报进预报单货物行与托盘;选的是等级不是库位,下单估价仍无仓储行** |
...
| 补货 / 库位优化 / 上架建议 | Logiwa · Microlistics | 判据一:解决长期存货、反复拣选的问题;我们是批次进出(CR #126 例外,2026-09-15 lead:声明底层的托盘在上架页只提示第一个空闲底层库位,主管仍自选) |
...
入库:  Inbound(客户门户或 协调员 建 ASN;无预报到货建临时收货单;一批货进两个仓 = 同一 Job 下两张 ASN)
        → 到货 → Receiving 逐行记实收/短溢/损坏
              └─ 实收 ≠ 预报:仍可收,自动生成 Discrepancy 异常 → 协调员队列
...
3. **库位校验(基础版):** 库位存在、属于当前仓库、未禁用;容量与规则校验不做。**CR #126(2026-09-15 lead)唯一的规则校验:声明底层的正常托盘上架到非底层存储库位须填原因;移库不校验。**
...
| 收货作业 | 按 ASN 逐行收,记实收/短溢/损坏与原因;托盘单元录实测长宽高重与托盘来源(客户自带 / 仓库木托 / CHEP / LOSCAM),系统按客户价目表阈值建议托盘类型;卡车散货记卸货托盘数;差异自动生成异常进 协调员队列 | **WMS-2** — "records any differences";价目表卸货费 / 托盘租赁 / 仓储分类的数据来源 |
...
- 实收 ≠ 预报时仍可收货,但生成 Discrepancy 异常进 协调员队列处理;无预报到货建临时收货单,协调员 确认后才可上架。
...
- 2026-09-14 CR #123:共用解析器的准确性加固(编码 GBK / UTF-16、分隔符、全角、表头别名、cm / mm 与单件重量 × 箱数、电话 / 邮编 / 州 规范化、逐行中文报错带行号与列名)对 B2b 的 ASN Excel 导入同样生效;客户自己的清单则走门户上传 → 订单 → 待建预报(§3 A4 注),不直接建 ASN。
...
客户门户申请上门提货(CHANGE_REQUESTS #125):客户在门户入库清单勾选需要上门提货并按清单行自选提货方案(客户价)→ 客服在待建预报审核,生成预报单时一并提交提货(客户所选方案存 asns.collection_preference)→ 最终报价在客户容差内自动按客户所选确认,超出则客户在门户预报入库页重新确认;客服不生成预报单则运输不动
...
7. **底层库位附加费(CR #126,2026-09-15 lead):** 托盘声明底层且周内最后一次快照在底层存储库位,加收 `WH-STORAGE-TIER-PLT-WK` = 该托盘当周基础仓储费 × 百分比(标准表 10 %,区间 10–20 % 存在 `threshold_json`,可按墨尔本 / 悉尼仓各设一行);散箱不计,基础费缺失 / POA 时附加费待审核,不出 $0。
