...
| **Q3** | **OMS-3 定价 = 服务费预估** —— 按客户价目表估算仓储/操作/运费,下单时可见 | 任务 A7b 调 `RateService` 做预览;订单行里的 `unit_price / total_price`(货值 AUD)只存不算,仅供报关与保险参考;**CR #126(2026-09-15 lead):单价可按客户价目表门槛 `tier_value_threshold_cents` 预选“底层”存储等级,仍不参与任何计价** |
