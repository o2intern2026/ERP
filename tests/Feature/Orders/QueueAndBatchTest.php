...
class QueueAndBatchTest extends TestCase
{
    ...
    public function test_batch_page_lists_every_order_generated_from_one_container(): void
    {
        ...
        $byContainer->assertSee($asn->asn_no)->assertSee($orderA->order_no)->assertSee($orderB->order_no)->assertDontSee($orderC->order_no)->assertSee(__('orders.batches.revenue'))->assertSee('$0.00'); // charges table exists since M6: revenue is a number, no longer pending (rendered via Money::format, tester feedback #9)
