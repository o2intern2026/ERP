...
return [
    ...
    'inbound' => [
        ...
        // CHANGE_REQUESTS #158 (revised 2026-09-25): the client types a number or presses the button.
        'container_generate' => '重新生成',
        'container_auto_hint' => '柜号已随机生成（CTN-6 位编码），可以直接改成船公司的实际柜号，或点“重新生成”换一个；提交后随清单交给客服，建预报单时仍可修改。',
        ...
        'template' => '下载 CSV 模板',
        'template_xlsx' => '下载 Excel 模板（与贵司拼箱清单格式完全一致）',
        'template_hint' => '模板就是贵司拼箱清单的原格式：第 1 行分组说明、第 2 行英文表头、第 3 行起一行 = 一箱（示例 3 行，请删掉再填）。原清单可以直接上传，不必改成模板；系统只读它认得的列，其余列（发货人、国家、街道、电池、邮箱、E 列尾程单号）保持原样即可。CSV 为 UTF-8 带 BOM，中文 Windows 的 Excel 可直接打开；中文表头的旧模板仍然能传。',
        'template_columns_title' => '表头说明（英文表头 = 含义；标“不读”的列保留即可）',
        'template_columns' => [
            "Sender's Name" => '发货人名称（不读）',
            "Sender's Address" => '发货人地址（不读）',
            "Sender's Phone" => '发货人电话（不读）',
            'ChannelWaybillNumber' => '运单号，作唛头（必填；同一收件人多箱时写 -1、-2……）',
            'E 列（无表头）' => '尾程快递单号，由拼箱方生成（不读，可留空）',
            'Country' => '国家（不读）',
            'State/Province' => '州（NSW、Queensland、Vic 等写法都可以）',
            'City' => '城区',
            'Suburb' => '街道（不读，城区取 City 列）',
            'Street Name' => '街道名（不读）',
            'Unit/Street Number' => '门牌号（不读）',
            'Recipient' => '收件人（必填）',
            "Recipient's Email" => '收件人邮箱（不读）',
            "Recipient's Phone Number" => '收件人电话（缺前导 0 会补回）',
            'Postal Code' => '邮编（必填）',
            'Detailed Address' => '详细地址（必填）',
            'Battery Type' => '电池类型（不读）',
            'Battery Packaging' => '电池包装（不读）',
            'Commodity' => '品名（中英文均可）',
            'TTL VALUE(AUD)' => '单件价（澳元）',
            '商品数量' => '件数',
            '每箱产品总价 (AUD)' => '每箱总价（澳元）',
            'Length(cm)' => '长 cm',
            'Width(cm)' => '宽 cm',
