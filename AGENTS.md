...
- **Server-rendered Blade only.** No npm, no build step, no Vue / React / SPA. Pico.css (CDN) + one `app.css` ≤ 100 lines. Inline vanilla JS per page; the only CDN libs are `html5-qrcode` and the signature canvas. Every warehouse form accepts scan-gun (keyboard) input. Date / time inputs (`type="date|datetime-local|month|time|week"`) carry `lang="en-AU"` right after the type so the native picker reads dd/mm/yyyy under the Chinese page language (guarded by `DateInputLangTest`).
