...
- **Server-rendered Blade only.** No npm, no build step, no Vue / React / SPA. Pico.css (CDN) + one `app.css` ≤ 100 lines. Inline vanilla JS per page; the only CDN libs are `html5-qrcode` and the signature canvas. Every warehouse form accepts scan-gun (keyboard) input. Date fields use the shared `<x-date-field name="…" :value="…" />` component (`resources/views/components/date-field.blade.php`, frozen zone): dd/mm/yyyy display in every browser, ISO `yyyy-mm-dd` submitted from its hidden input — the only named one — and the calendar picker behind a button, so controllers, validation and `old()` are untouched; never a raw `<input type="date">` (guarded by `DateFieldTest`). A prefill script sets the hidden input's value and dispatches `change`; `time` adds a native time box for an ETA (hidden value `Y-m-d\TH:i`). Plain `type="time"` inputs stay native.
